...
class InMemoryPredictor(StandardPredictor):
    """
    Applies the model on the given in-memory dataset and returns the result as an in-memory array.
    Predictions from the network are kept in memory.

    The output dataset names inside the results dictionary is given by `dest_dataset_name` config argument. If the
    argument is not present in the config 'predictions{n}' is used as a default dataset name, where `n` denotes the
    number of the output head from the network.
    ...
    """
    def __call__(self, test_loader):
        assert isinstance(test_loader.dataset, MemoryDataset)
        ...
        results_collection = self._save_results(prediction_maps, normalization_masks, output_heads, None, test_loader.dataset)
